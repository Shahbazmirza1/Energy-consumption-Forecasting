...
const SarimaX = () => {
  ...
  useEffect(() => {
    d3.csv('/pred.csv').then(data => {
      ...
      // Simulate SarimaX predictions with seasonal variations
      filteredData.forEach(d => {
        const month = d.timestamp.getMonth();
        const seasonalityFactor = 1 + 0.3 * Math.sin((2 * Math.PI * month) / 12);
        const trendFactor = 1 + 0.005 * (d.timestamp.getTime() - new Date(2018, 0, 1).getTime()) / (1000 * 60 * 60 * 24 * 365);
        d.predicted = d.PJME_MW * seasonalityFactor * trendFactor + (Math.random() * 0.1 - 0.05) * d.PJME_MW;
      });
